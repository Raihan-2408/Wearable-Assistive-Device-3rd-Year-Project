...
tts_lock = threading.Lock() # Prevents race conds

# VARIABLES TO TRACK LAST SPOKEN OBJECT AND TIME
last_spoken_objects = None
last_spoken_time = 0
SPEECH_DELAY = 5 # Adjust to prevent rapid speech spam before repeating same detection
MIN_SPEECH_GAP = 1 #gap between two speech events 

def speak_async(text):
    """Announces detected objects without delays or speech queueing issue"""
    
    global last_spoken_objects, last_spoken_time
    
    current_time = time.time()
    
    #Avoid repeating asame object too quickly 
    if text == last_spoken_objects and (current_time - last_spoken_time < SPEECH_DELAY):
        return

    
    #UPDATE LAST SPOKEN INFO
    last_spoken_objects = text
